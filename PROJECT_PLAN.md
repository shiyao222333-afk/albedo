...
> 当前版本：**v0.2.1（字幕内容线增强）** ｜ v0.2.0 完整多维鉴定报告已验收（2026-07-12）｜ v0.1.0 已验收（2026-07-09）
...
**当前重心**：v0.2.1 字幕内容线增强（在 v0.2.0 完整多维鉴定报告基础上，针对 B站等字幕类输入新增「分类 → 关键句锚定 → 高光窗口 → 按类型萃取 → 保真自检」管线；验真环节（真/假/可疑）研究下一轮启动，本轮 `assess.py` 零改）。
...
- v0.3.0：跨源比对引擎 + 共识统计 + 时效判定 + 冲突仲裁 + 领域上下文（← 实现参考：见 6.8 旧代码遗产，含可直接复用的 `ds_fusion.py` / `tms.py` / `claim_extractor.py`）
