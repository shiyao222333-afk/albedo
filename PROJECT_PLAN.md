...
三、主线演进路线        —— 逻辑推进：核心→丰富→规模（v0.1→v0.2→v0.3）
四、任务清单            —— 对齐路线，按版本分组
五、设计基础            —— 数据契约 / 技术选型 / 验收 / 竞品来源（支撑主线落地）
六、未来研究清单        —— ★独立区★ 尚未排期，待研究后并入对应版本
七、决策记录索引        —— ADR-002~005 一句话 + 文件
```

> **本文两条主线分明**：第三~五节是「现在要做的计划」（已排期）；第六节是「将来要研究的问题」（未排期，独立成区，不混入主线）。
...
**推进逻辑**：每版都建立在上一版已稳的能力之上，不跳步——
- **v0.1.0 先稳核心**：单条内容能不能信（真实性维度 + 净化），最小可用先跑通。
- **v0.2.0 再补完整**：在已稳的「真假」之上，补多维质量 + 优点 + SOP + 溯源 + 引用/变现 + 批量 + 入库就绪，产出完整报告。
- **v0.3.0 最后做规模**：等积累了多条来源可比，再做跨源矛盾检测 / 共识统计 / 时效 / 仲裁——这些没有「多条来源」无从做起，故放规模期一次做实。

| 版本 | 阶段 | 目标 | 关键能力 |
|---|---|---|---|
| **v0.1.0（当前）** | MVP | 验真假核心闭环 | 内容净化（按文本类型：字幕 ASR 清洗 / 结构化文案直提炼）+ 质量评估**真实性维度**（大模型 + 轻量统计数值自洽）；先跑通单条最小可用 |
| v0.2.0 | MVP 延伸 | 完整多维鉴定报告 + 批量 | 质量评估补全**文案/结构/逻辑**维度 + 优点分析（6 子能力）+ 结构化 SOP + 溯源 + **引用标记** + **变现标注** + **人类可读报告渲染** + 批量/队列并行 + **入库就绪(ingestion_meta)** |
| v0.3.0 | 规模期 | 验真假深化 | 跨源矛盾检测 + 跨源共识统计 + 时效判定 + 冲突仲裁（基础验真假已稳，规模期一次做实） |
...
> **本区与主线（第三~五节）严格分离**：以下不是当前路线的一部分，是已知待研究的课题。待规模期或单独研究后，再并入对应版本的任务清单（第四节）。带 ★ 为优先项（现在成本最低、对一人公司最有用），先埋种子、其余慢慢研究排期。
> 来源：2026-07-09 架构讨论「还有什么遗漏，头脑风暴一下」。用户指示：全部写入规划，之后再慢慢研究。

### 6.1 进料环节（东西进炼真之前）
- ★ **去重 / 近重复检测**：同视频采两次、不同 UP 讲同一件事→先去重，避免重复炼、白花钱。归属：v0.2.0+ 进料预处理 / 需 Nigredo 配合指纹。
- **超长内容切块**：2 小时课 vs 30 秒技巧长度差百倍，太长先切块再评，否则 LLM 丢上下文。归属：v0.2.0+ C1 前切分。
- **图里的信息丢了**：PPT 对比图、视频演示画面只吃文字，图内数据/证据完全没进炼真。归属：需上游 OCR/多模态，远期。
- **采不到的内容标注**：微信付费文、B站会员专享适配器抓不到，要标「采不到」而非假装没有。归属：Nigredo 适配器职责。

### 6.2 验真假本身的盲区
- **伪造证据**：AI 假截图、假数据图、深度伪造——统计只查文字数值，查不了图真假。归属：远期多模态。
- **域内外有别误伤**：酷家乐专属技巧在领域内真、放通用可能错；「真实性」可能误伤领域正确内容。归属：v0.3.0 加领域上下文。
- **全新说法无处印证**：刚出新方法网上无第二来源，跨源统计用不上，只能标「未印证」而非真假。归属：v0.1.0 起标「未印证」而非真假。
- **评分主观偏见**：文案/结构/逻辑分是 LLM 审美，非客观真理——报告要注明「这是风格分不是质量分」。归属：v0.2.0 报告注记。

### 6.3 提炼出的东西
- **SOP 未验证**：产出标准步骤但没真跑过，不知能否用/踩坑。归属：v0.3.0+ 与 Rubedo 闭环验证。
- **过时 SOP**：2023 方法 2026 可能失效（与时效判定重叠），产出时顺手标时效风险。归属：v0.3.0。
- **多冲突 SOP**：不同来源步骤打架，提炼阶段提示「存在冲突版本」（与冲突仲裁重叠）。归属：v0.3.0。

### 6.4 引用 / 变现的延伸
- **引用未核实**：标了「推荐《XXX》书/某网址」但没验证真存在、真说过他声称的话（可能被断章取义当权威）。归属：v0.2.0+ 引用核验。
- ★ **变现 ≠ 差内容护栏**：卖课不一定假、免费不一定真；变现标注不能和「可疑」画等号，报告分开呈现。归属：v0.1.0 起报告注记。
- **软广识别**：比硬卖课难的「软广」（看似分享实则种草），卖课话术库主要抓硬广。归属：v0.2.0+ 话术库扩展。

### 6.5 用起来的闭环
- **反馈回路**：你看完觉得「它判错了」，炼真能否记住纠正、下次更准。归属：规模期学习回路。
- **人肉覆盖**：你明知标「可疑」但觉得可用，能否一键「我信这个」并保留决定。归属：v0.2.0 UI。
- **批量总览看板**：炼了 100 条后有无「哪些最值得做」汇总，而非 100 份报告散着。归属：v0.2.0+。
- **成本账**：每条调几次 LLM、花多少钱——一人公司要算账。归属：v0.2.0+ 计量。

### 6.6 怎么知道炼真自己靠谱
- ★ **可解释到根因**：标「可疑」要能点开看「因为第 X 条证据不足 / 数值前后矛盾」，否则不敢信。归属：v0.2.0 报告钻取。
- **评测集**：需一小批你亲自标过「真/假/好」的样本当考题，定期打分证明炼真判得对。归属：规模期评测。

### 6.7 战略层可加的评分维度
- ★ **新颖度**：这条是老生常谈还是新打法？对你「新」往往更值钱。归属：v0.2.0+ 维度扩展。
- ★ **上手门槛 / 前置成本**：做这事要多少工具/技能/钱？帮你判断「我现在能不能做」。归属：v0.2.0+ 维度扩展。

> **排期原则**：★ 优先项在 v0.2.0 起即埋种子（报告注记 / 进料预处理 / 维度字段预留）；其余按上表归属版本，规模期或单独研究后落地。本区内容动态演进，不计入当前版本任务。
