...
> 当前版本：**v0.4.3（抽主张重建：形式信号骨架 CE0 + 自一致性抽主张 CE1+CE2 + 确定性忠实性自检 CE3 + 主张缓存 CE4 + Layer3 联网核查框架，已实现待提交）** ｜ v0.4.2 MiniCheck 真部署已提交 ｜ v0.4.1 §6.2 证据链判定已提交 ｜ v0.4.0 形式线已提交（2026-07-16）｜ v0.3.0 验真环节已提交（2026-07-16）｜ v0.2.1 字幕内容线增强已提交（2026-07-15）｜ v0.2.0 完整多维鉴定报告已验收（2026-07-12）｜ v0.1.0 已验收（2026-07-09）
...
八、用户未来设想          —— 2026-07-17 确认 4 点方向级设想，列入路线图
```
...
- **TT6** Layer2 联网深验 `core/truth_track.py` `verify_claims_web()`：已接入 **mDeBERTa-XNLI 本地验真**（`core/minicheck_verify.py` 已重写为 mDeBERTa，MiniCheck 路线弃用）；模型未下载时降级标 `unverified` 📍TT5 ✅ 已实现（2026-07-17 切换 mDeBERTa-XNLI，详见 `docs/RESEARCH-TRUTH-MODELS-CN-2026-07-17.md`）
...
### v0.4.3 抽主张重建（稳定性兜底，以 DeepSeek 不稳为前提）[MVP必须·已实现 2026-07-16]

> 版本说明：设计文档与原计划称本版为 "v0.4.2"，但 `v0.4.2` 已被 MiniCheck 真部署占用（commit 326abc0），故顺延为 **v0.4.3** 避免撞号。
> 背景：v0.4.4 鲁棒性实测暴露——同视频 3 遍 `n_claims=15/18/13`、`truth_label=suspect/false/true` 翻转。根因不是代码 bug（截断/崩溃已修于 v0.4.4 未提交改动），是 DeepSeek 不尊重 temperature=0，每次抽出主张集合不同 → 下游 D-S 输入变 → 结论翻。文献（ICLR2026 Dissecting Non-Determinism）证实即使 temp=0 仍不稳（GPU 浮点非结合性 + API 动态批处理），故"不稳"是前提不是 bug。
> 设计文档：`docs/DESIGN-CLAIM-EXTRACTION-2026-07-16.md`（已写，含形式信号骨架深研 + 预设参数）。
> 用户拍板 5 点：① 先做 #1 设计（深研形式信号骨架）② #2 标注评测集进路线图 ③ 程序跑起来优先、先给预设参数、#2 完成后调参 ④ 联网核查也要做 ⑤ 解释 D-S 并确认可用。
> 边界：与 `PROJECT_PLAN.md` 第 362 行定论不冲突——"seed+多采样投票对 DeepSeek 无效"针对**最终判定投票**；本版"自一致性"仅用于**抽主张 N 次并集去重**（非投票），且抽完缓存不重抽。

- ✅ **CE0** 形式信号骨架 新 `core/salience.py`：复用 `form_track.apply_rhetoric_rules`（FT4 规则单一来源）+ 伪声学（语速=字数÷句时长、停顿=句间隔、标点强调、字符频率中心度 TF），7 维加权融合算 `salience`（预设权重见设计 §3.4，和=1.0）→ Top-K（预设 K=12）候选高亮骨架（带 ts/原话/显著度/7 维信号标签/命中话术/重叠窗口上下文）。**零 LLM、确定性、<1s** 📍②前处理
- ✅ **CE1** 约束解码抽主张 `core/truth_track.py` `extract_claims_self_consistent()`：启用 `response_format=json_object`（`llm.py` 新增透传，沿用递增预算 4096→8192），提示固定 CE0 骨架为"必须覆盖的原话范围"，跑 N 次（预设 N=3）📍③
- ✅ **CE2** 自一致性并集去重：合并 N 次主张，文本归一化去重（非投票）→ `claims`；重叠窗口上下文防跨句主张漏抽 📍③出口
- ✅ **CE3** 忠实性自检 `core/truth_track.py` `faithfulness_check()`：每条主张 vs 字幕全文子串/模糊匹配（跨句子句拆分），标 `grounded` + 锚点 `anchor_ts`；`ungrounded` 丢弃（零 LLM，补 faithfulness gap）📍③出口
- ✅ **CE4** 主张缓存 新 `core/claim_cache.py`：`cache/{video_id}.claims.json` 抽完落盘，复查/出报告复用不重抽（消除同视频三次漂移）📍③后
- ✅ **Layer3** 联网核查框架 新 `core/web_verify.py`：对 Layer2 仍 `unverified` 的公开事实主张做联网升级；无 `ALBEDO_SEARCH_API_KEY` → 诚实降级标 `web_status=pending`（待联网核查），不臆断；可插拔 `backend` 接口待配 key 后接真搜索 📍③后
- ✅ **A5e** 编排接入 `flows/refine.py` + `truth_track._run_truth_track()`：②抽关键原话前插 CE0 骨架；③ `extract_claims` 换 CE1+CE2（无骨架退化旧路径）；出口接 CE3+CE4；确定性层（④MiniCheck/⑥D-S/⑦入库）不动
- **验收（确定性+链路测试已通过）**：`tests/test_claim_stability.py`（CE0 Top-K 降序 / CE1+CE2 并集去重 / CE3 grounded 丢弃幻影 / CE4 缓存一致）全 PASS；`tests/test_pipeline_smoke.py`（A5e 缓存命中直接载入 + 抽取写缓存 + Layer3 pending 降级）全 PASS。真实视频 3× 鲁棒性测试（烧 DeepSeek key）待用户提供字幕样本后跑（B站对该视频字幕接口需登录，沙箱免登录取不到）。
- **注**：预设参数先用（N=3/K=12/权重见设计文档 §3.4·§4.3），待 #2 标注评测集建成后回测调参；Layer2 本地验真模型已定 **mDeBERTa-XNLI**（最轻·多语言含中文·确定性，2026-07-17 用户拍板弃用 MiniCheck 改选）；未来 **B 类大模型级联核查**（DeepSeekFakeNews-LLM-7B 等作难例升级，见 6.2）待 #2 评测集建成后排期。


### v0.4.8 报告质量根治（11 问题 + 多轴分类 + 通用锚定闸门）[规划中 · 等 3×鲁棒性报告出来后实施]

> 背景：第一份真实视频报告（BV1pQ7o61EMh，mDeBERTa-XNLI 真验真）逐条审查暴露 11 个问题，根因分四族——幻觉/保真类（SOP 编造+假时间戳、钩子自相矛盾、关键原话/高光块空）、解析类（数值误报 income=02/03）、评分/标签类（包装强度文案写死"高包装"、单条吹牛主张→整篇"真实"、90 vs 0.6 两分数打架、表达力/干货度偏高）、抽取质量类（仅 3 条主张 2 条废话）。2026-07-17 用户拍板两点设计基调：①**分类与模板不能太简单**——视频类型多（教程/工具/卖课/SOP/纯娱乐/讲解…），须多轴综合考虑，不强制单标签；②**通用锚定闸门默认**：SOP 步骤编造直接剔除，主张/摘要编造标⚠️保留。竞品对标（已联网查）：grounding+citation 校验(novakit)、ClaimBuster checkworthiness、PolitiFact 6 级 + FActScore/SAFE 原子验证、Number sanity(vergex)。
> 用户关键纠正（2026-07-17）：**"揭秘卖课"含 SOP 不矛盾**——问题 3 错不在"被分成 tutorial"，而在单标签分类太粗 + SOP 萃取无锚定校验。故本版**不推翻分类**，而是改多轴 + 加闸门。

**A. 多轴分类（取代单标签 classify_content_type）**
- **AC1** `core/classify.py` 改 `classify_content(inp) -> {structure, intent[], monetization}`：
  - `structure`：步骤演示 / 叙事故事 / 观点论点 / 概念讲解 / 要点罗列 / 混合 / unknown（驱动萃取模板）
  - `intent`：**可多选**——教 / 告知 / 卖货 / 娱乐 / 说服 / 揭秘曝光（决定信任加权与声明）
  - `monetization`：复用 `assess_monetization` 输出（卖课/电商/工具/无）
  - 示例：「揭秘卖课+演示 agent 怎么搭」= structure=步骤演示 + intent=[卖货,揭秘曝光] + monetization=卖课

**B. 混合模板路由 + 卖家声明（取代 extract_by_type 的 switch 互斥）**
- **AC2** `core/content_track.py` `extract_by_type` 改：structure 决定抽什么（步骤演示→必抽 SOP）；intent 含[卖货/揭秘曝光]→SOP 上加"卖家/揭秘方演示，需独立验证，不代表已确证可行"声明，且**不抬高干货度/信任度** 📍C7
- **AC3** 报告 `_render_content_extract`：干货度 `_content_score` 改以"锚定通过的条目数"计，幻觉/未锚定条目不计入 📍A4

**C. 通用锚定闸门（治本：扩展 form_track G2 + content_track.check_grounding 到所有萃取）**
- **AC4** 新 `core/ground_extract.py` `gate_extract(items, subtitle_lines)`：每条带 ts/原文 → 程序化核对 ts 是否在真实字幕时间戳、原文是否字幕子串；返回 grounded/ungrounded 📍C7出口
- **AC5** 接入 SOP 步骤 / 大纲要点 / 主张：
  - SOP 步骤 ungrounded → **直接剔除**（编造步骤留着误导）
  - 主张 / 摘要 ungrounded → **标⚠️保留**（供人眼判断）
  - `_extract_sop` 提示词加："只抽视频真演示/讲解的步骤；仅口头描述没真做→输出'未提供可照搬步骤'，勿编造" 📍C7

**D. 快修（纯渲染/解析，低风险）**
- **AD1** `report.py:752` 包装强度文案条件化：≥0.7 才写"高包装"，否则"低/中包装" 📍A4
- **AD2** `assess.py:241` 数值扫描前剥离 `[mm:ss]` 时间戳（根治 income=02/03 误报）📍C3
- **AD3** `report.py` 钩子/叙事段若被 form_faithfulness 标 ungrounded → 渲染时标"⚠️ 非逐字/无字幕依据"，不伪装权威原句 📍A4
- **AD4** 关键原话遇截断自动升 max_tokens 重试；高光块输入为空→报告写"未提供高光数据"而非"未能生成" 📍A4/C7

**E. 主张抽取 checkworthiness 过滤（对标 ClaimBuster）**
- **AE1** `truth_track.extract_claims` 加 checkworthiness 前置：只保留"可证伪的事实主张"，过滤过渡句/水词/口号（"接下来全程干货""一听就懂"类）📍③

**F. 真实度标签语义重构（对标 PolitiFact / FActScore）**
- **AF1** `judgment.judge_document`：标签区分"来源自洽(supported-by-source)"与"外部已验证(verified-true)"；在 Layer3 未做前，supported 主导的"真实"改标"视频自证一致（待外部核实）"，避免误导 📍⑥
- **AF2** 结论卡统一两分数：要么只用一口径，要么显式分两行"来源自洽度 X / 外部验证度 Y（待联网）"，消除 90 vs 0.6 打架 📍A4

**竞品对标表（本版直接采用）**
| 竞品做法 | 落地点 |
|---|---|
| grounding + citation 程序化校验 + 二次 verifier（novakit/modeldatabase） | AC4/AC5 通用锚定闸门 |
| ClaimBuster checkworthiness 三分类（NFS/UFS/CFS） | AE1 主张过滤 |
| PolitiFact 6 级 + FActScore/SAFE 原子验证 | AF1/AF2 标签语义 |
| Number sanity（解析前剥离时间戳）（vergex） | AD2 数值误报 |

**验收**：3× 鲁棒性报告（BV1pQ7o61EMh run1/2/3 已生成）对比——重点看 SOP 是否仍编造（问题3）、主张集漂移（问题7）、标签稳不稳（问题5/6）；`tests/test_pipeline_smoke.py` 回归全 PASS；真实视频复跑报告无"未能生成"/无 income 误报/无编造 SOP。
...
| ★ **联网深验 Layer3（外部证据升级判定）** | **规模期（v0.5.0 后，#2 评测集之后）** | 对 `grounded=False` 或 MiniCheck=unsupported 的**公开事实主张**，检索外部证据（搜索 API / ClaimReview 事实核查库 / 熔知知识库）取 top-k 片段作 docs → MiniCheck(claim, 片段) 判 supported/refuted → 把"视频无证据"升级为"网上查了是真的/假的"。属蓝图"规模期"能力（非 MVP），优先级低于抽主张稳定性(v0.4.2)与评测集(#2)。需先解决：检索源选择、中文检索片段质量、与 D-S 融合的接入点（把 Layer3 结论作为新证据源喂 §6.2 D-S） |
| ★ **B 类大模型级联核查（置信度路由 + 难例升级）** | **规模期（#2 评测集建成后，与 Layer3 联网深验同期演进）** | 当前 Layer2 用 **mDeBERTa-XNLI**（便宜·确定·全量）逐条验真，但本地 NLI **只比字幕**，判不了"需外部世界知识"的断言（如"月流水五六百万是否靠谱"）。引入 **B 类生成式大模型**（DeepSeekFakeNews-LLM-7B 等）作**升级核查**：mDeBERTa 置信度 ≥0.8 直接采纳；低置信 / neutral 的难例升级给 B 类大模型 + 联网检索（Layer3）做深度核查，最后 Judge 证据融合出文档结论。行业实践（DeepSciVerify 证据升级 / PCC 置信度自适应路由 / LLM Cascade 研究）证实时可省 **up to 79.5% 成本且保 90% 质量**（前提：校验器便宜且确定——mDeBERTa 正好符合）。B 类也作**标注集生成器**反哺小模型（造训练数据提升中文 NLI 准确率）。**不当 Layer2 主角**（慢/贵/每次生成非绝对确定），只当"疑难顾问 + 数据工厂"。候选与比较计划见 `docs/RESEARCH-TRUTH-MODELS-CN-2026-07-17.md`。 |
...
| ★ **#2 标注评测集（设计 + 建设）** | **设计进 v0.4.2 路线图 / 建设 v0.5.0** | 需一小批用户亲自标过「真/假/好」的样本当考题，定期打分证明炼真判得对。**设计文档待写**（用户已批准：先设计"评测集长什么样"——样本量 10~20 条起步、每条记字段：主张原文/真·假·存疑/视频内有无证据支撑/出自哪个 ts；存储格式便于自动比对；打分规则）。**它同时是 v0.4.2 预设参数的调参依据**——#2 建成后回测调 CE0 权重与 CE1 的 N/K（用户拍板"2完成后调整参数"）。建设与验收：在带标准答案的数据上测整条流水线准确率/召回率，而非仅看标量标签 |
...
## 八、用户未来设想（2026-07-17 确认，列入路线图）

> 来源：用户于 2026-07-17 提出的 4 点方向级设想，已逐条确认「全部列入路线图」。
> 定位：高于「版本任务」的**方向级**设想，不替代第六节的 22 项已排期课题；其中多项与现有计划条目对齐（见各条「与现有计划关系」），本节能把它作为用户长期方向固化、便于将来进入对应版本时展开任务。

### 设想一：多平台入口扩展（从「只认 B站」到多源）
- **用户原话**：现在只支持 B站，未来要支持小红书文案等。
- **含义**：炼真入口从「仅 B站视频字幕」扩展到小红书文案、文章、网页等多源；进门前由各平台适配器统一转成文字 + 文本类型标记，再走同一套分析。
- **与现有计划关系**：对齐 5.1 数据契约 `text_type`（已含 `social_post`/`article`/`webpage`）+ 6.1「多题材/多文件格式兼容」（v0.5.0 起）+ BLUEPRINT §二⑤（平台无关、认文本类型）。本设想把它**明确固化**为长期方向——小红书等为首批扩展目标。
- **落点版本**：v0.5.0 起（随 Nigredo 适配器扩展）+ v1.1.0 多适配器对接。

### 设想二：分析方法跨文件类型复用（一次积累，多处通用）
- **用户原话**：分析方法对不同文件原理应该共用，比如文章分析也可以分内容和形式；现在 B站视频分析是技术积累，未来不用完全重新开发。
- **含义**：分析方法是「平台/文件类型无关」的，入口才平台相关。B站视频分析是先做的「技术积累/练兵」，文章、文案、网页来了复用同一套「内容线 + 形式线 + 验真线」三轨框架，不从头写。
- **与现有计划关系**：对齐 BLUEPRINT §二⑤（炼真不绑平台、认文本类型）+ 2.2 护城河②（平台无关认知精炼 + 文本类型感知）+ A2.3 结构类型注册表（插拔即扩）+ 6.1 多题材兼容。本设想确认「三轨框架的平台无关性」是长期架构原则。
- **落点版本**：架构原则（现在即遵循），扩展随 v0.5.0 多题材兼容 + v1.1.0。

### 设想三：与熔知知识库联动做「矛盾检测」
- **用户原话**：随着熔知知识库里面内容越来越多，未来炼真的验证环节需要和知识库里面的内容比较有没有矛盾的地方。
- **含义**：验真从「孤立判断这条对不对」升级为「和已知知识比，看打不打架」——拿新内容与熔知库已有内容比对矛盾。
- **与现有计划关系**：**正是** BLUEPRINT §二「将来会做 · 跨来源矛盾检测」+ 6.2 / 6.8（v0.3.0 跨源比对引擎、冲突仲裁，旧代码 `D:\albedo-old` 现成地基）+ v0.5.0 后联网深验 Layer3（检索源含熔知知识库）。用户已确认归类无误。
- **落点版本**：规模期（v0.3.0 跨源比对引擎预留 → v0.5.0 后 Layer3 接熔知库）→ v1.1.0 五器合体。
- ⚠️ 跨项目：需炼真↔熔知接口（归「五器耦合」阶段，等两边各自可用后做，遵守跨项目边界铁律）。

### 设想四：增强「内容 + 形式」分析法，方便学习模仿
- **用户原话**：未来进一步增强内容和形式分析的方法，方便学习模仿。
- **含义**：从「判断好坏」升级到「教人怎么做好」——把「好在哪 / 怎么做到的」讲得更透、更结构化、可直接照抄照学（深化优点分析的形式轴 `format_reusable` / `presentation_craft` + 内容轴可复用步骤）。
- **与现有计划关系**：对齐 A1.4/A1.5 形式轴（表达形式质量 / 格式可复用）+ 2.1 竞品借鉴（skill-from-masters 方法萃取）+ 6.7 新颖度/门槛维度（间接支撑「值不值得学」）。本设想把它明确为「分析报告可学习性」的长期深化方向。
- **落点版本**：v0.6.0 智能维度（新颖度/门槛）+ 持续深化优点分析方法。

---

> 记录方式说明：以上 4 条为**方向级**设想，已确认列入路线图；具体任务拆解在对应落点版本进入时展开（沿用第四节模式），不在此预先拆任务，避免与第六节 22 课题重复排期。

---

## 九、研究文档索引（docs/ 全部研究成果）

> 炼真的每个能力都由调研 / 设计 / 审计文档驱动，全部存于 `docs/`。本索引按主题汇总，便于追溯「某次改动背后依据了什么研究」。**新增研究文档请在本节补登记行**（文件名 + 日期 + 一句话主题）。

**一、立项与遗留代码**
| 文档 | 日期 | 主题 |
|---|---|---|
| `ALBEDO-RESEARCH-2026-07-09.md` | 2026-07-09 | 立项调研：对标 pangu / nuwa / OpenFactCheck / TubeScribed + 竞品全景，划定炼真边界 |
| `ALBEDO-LEGACY-CODE-ANALYSIS-2026-07-09.md` | 2026-07-09 | 遗留代码梳理（含 mDeBERTa 同款 `nli_detector` 已跑通线索） |

**二、内容线研究**
| 文档 | 日期 | 主题 |
|---|---|---|
| `RESEARCH-CONTENT-TRACK-2026-07-15.md` | 2026-07-15 | 内容线设计：分类 / 关键句锚定 / 按类型萃取 / 保真自检依据 |
| `RESEARCH-CONTENT-SUMMARY-2026-07-15.md` | 2026-07-15 | 内容摘要（中性"讲什么"）设计 |
| `RESEARCH-DATA-ANALYSIS-2026-07-15.md` | 2026-07-15 | 数据 / 竞品分析（内容维度） |
| `RESEARCH-DATA-ANALYSIS-TWOTRACKS-2026-07-15.md` | 2026-07-15 | 双轨（内容 + 形式）数据对照分析 |

**三、形式线研究**
| 文档 | 日期 | 主题 |
|---|---|---|
| `RESEARCH-FORM-TRACK-2026-07-16.md` | 2026-07-16 | 形式线：钩子 / 叙事 / 节奏 / 人设 / 修辞 / 模板 / 情绪客观拆解方法 |

**四、验真方法研究（5 篇）**
| 文档 | 日期 | 主题 |
|---|---|---|
| `RESEARCH-TRUTH-VERIFICATION-2026-07-15.md` | 2026-07-15 | 验真方法论 V1（真假二分 + 统计手段） |
| `RESEARCH-TRUTH-VERIFICATION-V2-2026-07-15.md` | 2026-07-15 | 验真方法论 V2（引入证据链雏形） |
| `RESEARCH-TRUTH-VERIFICATION-V3-GAPS-2026-07-15.md` | 2026-07-15 | 验真方法论 V3 + 暴露的缺口（GAP 清单） |
| `RESEARCH-TRUTH-METHODS-AUDIT-2026-07-16.md` | 2026-07-16 | 验真方法审计：对照 PolitiFact / FActScore / SAFE 找差距 |
| `RESEARCH-TRUTH-STABILITY-QUALITY-2026-07-16.md` | 2026-07-16 | 验真稳定性与质量评估（抖动根因分类） |

**五、中文验真模型选型**
| 文档 | 日期 | 主题 |
|---|---|---|
| `RESEARCH-TRUTH-MODELS-CN-2026-07-17.md` | 2026-07-17 | 8 个中文友好验真候选对比，选定 mDeBERTa-XNLI 依据 + 未来比较计划 |

**六、主张抽取设计**
| 文档 | 日期 | 主题 |
|---|---|---|
| `DESIGN-CLAIM-EXTRACTION-2026-07-16.md` | 2026-07-16 | 主张抽取设计 V1（自一致性并集 + CE0 骨架约束） |
| `DESIGN-CLAIM-EXTRACTION-V2-2026-07-16.md` | 2026-07-16 | 主张抽取设计 V2（补 checkworthiness 过滤 + 去重） |

**七、韧性 LLM 设计**
| 文档 | 日期 | 主题 |
|---|---|---|
| `DESIGN-RESILIENT-LLM-2026-07-16.md` | 2026-07-16 | 韧性层设计：空响应 / 截断 / 漂移根因与对策 |
| `REVIEW-RESILIENT-2026-07-17.md` | 2026-07-17 | 韧性层代码审查报告（火眼眼格式） |

**八、过滤器审计**
| 文档 | 日期 | 主题 |
|---|---|---|
| `FILTER-AUDIT-2026-07-17.md` | 2026-07-17 | 过滤器审计：盘点全部硬删点，确立「隔离不销毁」原则 + 硬删留痕审计 |
| `OVERVIEW-AUDIT-FIXES-2026-07-17.md` | 2026-07-17 | 审计修复总览（CE3 标记不删 / guard 回退 / 信任分反转等） |
| `OVERVIEW-CE0-AUDIT-2026-07-16.md` | 2026-07-16 | CE0 形式信号骨架审计（前倾偏差 + 时间桶覆盖） |
| `AUDIT-DELIVERY-GAP-2026-07-16.md` | 2026-07-16 | 交付缺口审计（报告维度缺失 / 降级不透明） |

**九、架构决策记录（ADR）**
| 文档 | 日期 | 主题 |
|---|---|---|
| `ADR-002-SCOPE-EXPANSION.md` | 2026-07-09 | 范围扩展决策 |
| `ADR-003-OUTPUT-FORM-AND-ANNOTATION.md` | 2026-07-09 | 输出形式与标注决策 |
| `ADR-004-DROP-DUAL-OUTPUT.md` | 2026-07-09 | 取消双输出（只交付单报告） |
| `ADR-005-INGESTION-READY-REPORT.md` | 2026-07-09 | 入库就绪报告（`ingestion_meta` 预填熔知分面） |

**十、状态记录**
| 文档 | 日期 | 主题 |
|---|---|---|
| `STATUS-2026-07-17.md` | 2026-07-17 | 当日状态快照（进度 / 待办 / 铁律） |
