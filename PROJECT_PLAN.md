...
> 当前版本：**v0.3.0（验真环节：逐条断言验真）** ｜ v0.2.1 字幕内容线增强已提交（2026-07-15）｜ v0.2.0 完整多维鉴定报告已验收（2026-07-12）｜ v0.1.0 已验收（2026-07-09）
...
**当前重心**：v0.3.0 验真环节（逐条断言验真）落地——在 v0.2.1 内容线基础上，把"验真假"从整条视频单源打分升级为"逐条断言验真"：Layer0.5 防 AI 瞎编断言 + Layer1 不联网快筛（绝对化话术 / 视频自相矛盾 / 时效标记 / 事实vs观点·个人vs公开分类）+ 预留 Layer2 联网深验（MiniCheck 本地）；结果逐条进报告、文档级信任分与入库状态直送熔知。OCR 跨模态、UP 主跨视频信用累积本期只定义字段、列入路线图（用户拍板）。
