...
> 当前版本：**v0.4.0（形式线 Track B：怎么讲的）** ｜ v0.3.0 验真环节已提交（2026-07-16）｜ v0.2.1 字幕内容线增强已提交（2026-07-15）｜ v0.2.0 完整多维鉴定报告已验收（2026-07-12）｜ v0.1.0 已验收（2026-07-09）
...
**当前重心**：v0.3.0 验真环节（逐条断言验真）已落地并入库（Layer0.5 防瞎编 + Layer1 话术/自相矛盾/时效/事实观点·个人公开分类 + Layer2 MiniCheck 接口预留；OCR/跨视频人设仅定义字段）。**现进入 v0.4.0 形式线（Track B：怎么讲的）**——在三轨正交架构中补齐"形式线"，分析内容"怎么讲"：钩子 / 叙事结构 / 节奏 / 人设 / 修辞话术（单一来源，truth_track 改为消费）/ 可复制模板（机器可读供凝华）/ 情绪曲线（弹幕弱代理），并通过 G1 反向桥（高说服包装+未验证→验真降分）与验真线联动、G2 形式保真自检防 LLM 编结构；报告升级"三轴总览（干货度/可信度/表达力）"。OCR 跨模态、UP 主跨视频人设累积本期只定义字段、列入路线图（用户拍板）。
...
> 铁规矩：`assess.py` 真实性评估零改（整体参考保留，验真结论上调 status）；全程 try/except 降级续跑；确定性 temperature=0 + 固定枚举。
...
- **TT6** Layer2 联网深验接口 `core/truth_track.py` `verify_claims_web()`：MiniCheck 本地接口预留，沙箱/未部署标 `unverified`（保守，V3 遗漏5）；真实启用路径已注释 📍TT5 ✅ 已实现
...
> 铁规矩：`assess.py` 真实性评估零改；全程 try/except 降级续跑；确定性 temperature=0 + 固定枚举；形式轴绝不参与 trust_score（G1 反向桥仅在"高包装+未验证"时下调验真分，属保守护栏）。
...
- v0.3.0：✅ **逐条断言验真已落地**（Layer0.5 防瞎编 + Layer1 话术/自相矛盾/时效/事实观点·个人公开分类 + Layer2 MiniCheck 接口预留；结果逐条进报告、文档级信任分与入库状态直送熔知）；跨源比对引擎 + 共识统计 + 冲突仲裁 + 领域上下文**仍排规模期子阶段**（← 实现参考：见 6.8 旧代码遗产，含可直接复用的 `ds_fusion.py` / `tms.py` / `claim_extractor.py`）
