...
def _edit_distance(a: str, b: str) -> int:
    """简单 Levenshtein 编辑距离（短串，O(len(a)*len(b))）。"""
    if a == b:
        return 0
    la, lb = len(a), len(b)
    if la == 0:
...
def _fuzzy_hit(q: str, s: str) -> bool:
    """q(主张) 是否为 s(字幕行) 的近义片段：容忍 1~2 字 ASR/改写偏字。

    数字安全闸：主张含数字时，要求匹配窗口的数字序列 == 主张数字序列，
    否则视为数字篡改（如"赚100万"不匹配"赚1000万"），判不命中，防篡改漏过。
    """
    if not q or not s:
        return False
    q_digits = re.sub(r"\D+", "", q)
    has_digit = bool(q_digits)
    qm = re.sub(r"\d+", "#", q)
    sm = re.sub(r"\d+", "#", s)
    # 快速路径：掩码后子串命中（数字须完全一致，否则可能数字篡改）
    if qm in sm or sm in qm:
        if has_digit and q_digits != re.sub(r"\D+", "", s):
            return False
        return True
    # 滑窗近义匹配（主张应为字幕行某片段附近）
    L = len(q)
    for wl in range(max(1, L - 2), L + 3):
        if wl > len(s):
            continue
        for start in range(0, len(s) - wl + 1):
            w = s[start:start + wl]
            wm = re.sub(r"\d+", "#", w)
            if wl == L and sum(1 for a, b in zip(q, w) if a != b) <= 1:
                if has_digit and re.sub(r"\D+", "", w) != q_digits:
                    return False
                return True
            if _edit_distance(qm, wm) <= 2:
                if has_digit and re.sub(r"\D+", "", w) != q_digits:
                    continue
                return True
    return False
...
def _substring_match_ts(quote: str, subs: list) -> Optional[str]:
    ...
    v0.4.x #244：精确子串失败后再走 _fuzzy_hit 模糊兜底（容忍 ASR/改写 1~2 字偏字，
    数字安全闸拦截数字篡改），避免忠实改写主张被误判 ungrounded → Layer0.5 误杀。
    """
    ...
    for ts, ntext in norm_subs:
        if ntext and norm_quote in ntext:
            return ts
    # 模糊匹配兜底（仅对不太长的主张，控制成本）
    if len(norm_quote) <= 40:
        for ts, ntext in norm_subs:
            if ntext and _fuzzy_hit(norm_quote, ntext):
                return ts
    # 跨句：拆子句分别匹配（允许分散在不同句；模糊兜底）
    ...
    if len(clauses) >= 2:
        ...
        for cl in clauses:
            hit = False
            for ts, ntext in norm_subs:
                if ntext and (_fuzzy_hit(cl, ntext) if len(cl) <= 40 else cl in ntext):
...
def guard_claim_faithfulness(claims: list, subtitle_lines: list, llm_kwargs: dict = None):
    ...
    v0.4.x #244：CE3 已确定性锚定（faithfulness=="grounded"）的忠实主张直接保留，
    不再走 LLM NLI 复核 —— 避免 LLM 噪声把"忠实改写主张"误杀（原"0 条主张"根因）。
    仅对 CE3 未锚定（ungrounded）的主张跑 LLM NLI 裁决。
    无字幕（通用路径）→ 无法核查，全部保留（faithfulness 保持 grounded 默认）。
    返回 (kept_claims, n_dropped)。LLM 失败 → 全部保留（降级不丢数据，与改动前一致）。
    """
    claims = claims or []
    if not claims:
        return [], 0
    ...
    # CE3 已确定性锚定（grounded）：直接保留，跳过 LLM NLI（避免误杀忠实改写主张）
    kept = [c for c in claims if c.get("faithfulness") == "grounded"]
    ungrounded = [c for c in claims if c.get("faithfulness") != "grounded"]
    if not ungrounded:
        return kept, 0

    subs_text = "\n".join(f"[{s.get('ts', '')}] {s.get('text', '')}" for s in subs)
    claims_text = "\n".join(f"{c['claim_id']}: {c['quote']}" for c in ungrounded)
    user = f"字幕原文：\n{subs_text}\n\n待核查断言：\n{claims_text}"
    try:
        data = call_llm_json(
        ...
        for c in ungrounded:
            ...
            if sup is None:
                # 回退：ungrounded 且无 NLI 结论 → 保守丢弃（防瞎编默认不放行）
                sup = False
            if sup:
        ...
        return claims, 0  # 降级：保留全部（与改动前一致，不丢数据）
...
def _run_truth_track(
    ...
    *,
    ...
    if cached is not None:
        # 缓存命中：漏斗数字必须与冻结的审计块(dropped_audit)同源，
        # 否则会出现"抽出2剔0"却列 11 条被删记录的自相矛盾（A 类报告 bug）。
        kept = cached["claims"]
        dropped_audit = cached.get("dropped_audit") or []
        claims_ae1_dropped = sum(1 for d in dropped_audit if d.get("stage", "").startswith("AE1_"))
        l0_dropped = sum(1 for d in dropped_audit if d.get("stage", "").startswith("L0.5_"))
        ce_dropped = sum(1 for d in dropped_audit if d.get("stage", "").startswith("CE3_"))
        n_extracted = len(kept) + len(dropped_audit)
    else:
        dropped_audit = []
        ...
        n_extracted = len(claims)  # #247 漏斗：AE1 前的抽取主张总数
    ...
    # #247 漏斗：让"误杀≠防瞎编"在报告可见，并对验真实质失效（抽出全被删）告警
    funnel = {
        "extracted": n_extracted,
        "ae1_dropped": claims_ae1_dropped,
        "ce3_ungrounded": ce_dropped,
        "l0_dropped": l0_dropped,
        "kept": len(kept),
    }
    if n_extracted > 0 and len(kept) == 0:
        funnel["alarm"] = (
            f"⚠️ 验真漏斗告警：抽出 {n_extracted} 条主张但全部被过滤"
            f"（AE1 剔 {claims_ae1_dropped} / CE3 未锚定 {ce_dropped} / Layer0.5 删 {l0_dropped}），"
            f"无法得出任何验真结论——请检查字幕质量或抽取提示词，勿误读为'无主张可验'。"
        )
    truth_track["funnel"] = funnel
