...
def _run_truth_track(
    ...
    *,
    ...
    if cached is not None:
        ...
        claims_ae1_dropped = 0
        if claims:
            filtered = [c for c in claims if c.get("check_worthy")]
            claims_ae1_dropped = len(claims) - len(filtered)
            claims = filtered
            if claims_ae1_dropped:
                logger.info("AE1 可证伪性闸门：剔除 %d 条非可证伪主张", claims_ae1_dropped)
        ...
        if cache_enabled and video_id and kept:
            save_claim_cache(video_id, kept, form_track=form_track, verify_sig=sig)

    truth_track = aggregate(kept, ce_dropped + l0_dropped + claims_ae1_dropped, persuasion_polish=persuasion_polish)
    return {"claims": kept, "truth_track": truth_track, "dropped": ce_dropped + l0_dropped + claims_ae1_dropped}
