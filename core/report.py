...
# refined_status 枚举映射（AI 设计提案，已由用户决策①"统一"确认，2026-07-17）：
...
def build_ingestion_frontmatter(out) -> dict:
    ...
    # 决策 Y：is_personal（是否个人经验）—— 炼真实产（truth_track.is_personal，强信号）
    if "is_personal" in tt:
        fm["is_personal"] = bool(tt.get("is_personal"))

    # 决策 Y + Q1=b：temporal_nature（时效）—— 炼真实产全 3 值
    #   transient：标题/关键词/摘要含"热榜/今日/限时/活动/直播/速报/突发/快讯"等短期信号
    #   timeboxed：含限时断言（n_timeboxed>0）
    #   evergreen：其余长期有效
    if tt:
        transient_markers = ("热榜", "今日", "限时", "活动", "直播", "速报",
                             "突发", "快讯", "每日", "周报", "早报", "日报")
        inp_title = inp.get("title") if isinstance(inp, dict) else getattr(inp, "title", "")
        haystack = " ".join([_str(inp_title), " ".join(kws), gist])
        if any(m in haystack for m in transient_markers):
            fm["temporal_nature"] = "transient"
        elif tt.get("n_timeboxed", 0) > 0:
            fm["temporal_nature"] = "timeboxed"
        else:
            fm["temporal_nature"] = "evergreen"
